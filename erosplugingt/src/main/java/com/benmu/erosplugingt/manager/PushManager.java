...
public class PushManager extends Manager {
    ...
    public void handlePush(Context context, String data) {
        ...
        boolean isForeground = this.isAppForeground();
    ...
    private void showNotification(Context context, NotificationBean bean) {
        if (bean == null) return;
        int iconId = ResourceUtil.getMipmapId(mContext.getPackageName(), "ic_launcher");
        String appName = context.getResources().getString(ResourceUtil.getStringId(mContext.getPackageName(), "app_name"));
    ...
    private boolean isAppForeground(){
        ...
        for(ActivityManager.RunningAppProcessInfo info : appProcessInfos){
            //当前应用处于运行中，并且在前台
            if(info.processName.equals(mContext.getPackageName()) && info.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND){
                return true;
            }
        }
        return false;
